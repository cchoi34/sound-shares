...
export function getSingleComposition(component, id) {
    try {
        const rootRef = firebase.database().ref();
        const compositionRef = rootRef.child("compositions");
        const singleCompositionRef = compositionRef.child(`${id}`);
...
export function postComposition(state) {
    ...
    const defaultImageURL = defaultImages[randomImage];
    const defaultName = "no-name-provided";

    const name = state.name || defaultName;
    const imageURL = state.imageURL || defaultImageURL;
